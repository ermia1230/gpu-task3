...
int mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    ...
    int fieldSize = grd->nxn * grd->nyn * grd->nzn;
    FPfield *d_Ex_flat, *d_Ey_flat, *d_Ez_flat;
    FPfield *d_Bxn_flat, *d_Byn_flat, *d_Bzn_flat;
    cudaMalloc(&d_Ex_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_Ey_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_Ez_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_Bxn_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_Byn_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_Bzn_flat, fieldSize * sizeof(FPfield));
    
    
    FPfield *d_XN_flat, *d_YN_flat, *d_ZN_flat;
    cudaMalloc(&d_XN_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_YN_flat, fieldSize * sizeof(FPfield));
    cudaMalloc(&d_ZN_flat, fieldSize * sizeof(FPfield));
    ...
    cudaMemcpy(d_Ex_flat, field->Ex_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Ey_flat, field->Ey_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Ez_flat, field->Ez_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Bxn_flat, field->Bxn_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Byn_flat, field->Byn_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Bzn_flat, field->Bzn_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    
    
    cudaMemcpy(d_XN_flat, grd->XN_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_YN_flat, grd->YN_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_ZN_flat, grd->ZN_flat, fieldSize * sizeof(FPfield), cudaMemcpyHostToDevice);
    
    
    struct particles d_part_data = *part;
    d_part_data.x = d_x;
    d_part_data.y = d_y;
    d_part_data.z = d_z;
    d_part_data.u = d_u;
    ...
    struct EMfield d_field_data = *field;
    d_field_data.Ex_flat = d_Ex_flat;
    d_field_data.Ey_flat = d_Ey_flat;
    d_field_data.Ez_flat = d_Ez_flat;
    d_field_data.Bxn_flat = d_Bxn_flat;
    d_field_data.Byn_flat = d_Byn_flat;
    d_field_data.Bzn_flat = d_Bzn_flat;
    
    struct grid d_grd_data = *grd;
    d_grd_data.XN_flat = d_XN_flat;
    d_grd_data.YN_flat = d_YN_flat;
    d_grd_data.ZN_flat = d_ZN_flat;
    ...
    cudaMemcpy(d_part, &d_part_data, sizeof(struct particles), cudaMemcpyHostToDevice);
    cudaMemcpy(d_field, &d_field_data, sizeof(struct EMfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_grd, &d_grd_data, sizeof(struct grid), cudaMemcpyHostToDevice);
